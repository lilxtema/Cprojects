...
int main() {
    ...
    int first_score = 0, second_score = 0;           // начальный счет
    ...
    while (1) {
        ...
        if ((ch == 'A' || ch == 'a') && player1) {
            ...
            if (leftP == 2)  // не делаем движение вверх, если центр ракетки стоит на 2 строке
                leftP -= 0;  // перемещение в верхнюю строку
                     ...
                     3)  // делаем движение только на одну строку вверх, если центр ракетки стоит на 3 строке
        ...
        if ((ch == 'z' || ch == 'Z') && player1) {
            player1 = 0;
            player2 = 1;
            if (leftP == 22)  // перемещает ракетку вниз на 1 строку,если она находится на строке 23
                leftP += 1;
            else if (leftP == 23)  // не двигает ракетку, если она стоит на строке 24
                leftP += 0;
            else  // движение на 2 позиции вниз при нажатии команды если нет доп.условии
                leftP += 2;
            draw_area(leftP, rightP, ballX, ballY, first_score, second_score, player1);
        }
        ...
        if ((ch == 'm' || ch == 'M') && player2) {
            ...
            if (rightP == 22)  // перемещает правую ракетку вниз на 1 строку,если она находится на строке 23
                rightP += 1;
        ...
        // проверяем на столкновение с правой/левой стеной и если шар столкнулся – меняем направление на
        // противоположное
...
void draw_area(int leftP, int RightP, int ballX, int ballY, int first_score, int second_score, int player1) {
    const int height = 25, width = 80;  // определяем границы поля
    printf("\033[0d\033[2J");  // при каждом вызове функции очищаем экран /033 предупреждает, что след идет
                               // команда
    ...
    for (int y = 0; y <= height; y++) {
        for (int x = 0; x <= width; x++) {
            ...
            if ((x == 1 && y == leftP) || (x == 1 && y == leftP + 1) || (x == 1 && y == leftP - 1)) {
                printf("|");
                flag = 1;
            }

            // ракетка правого игрока
            if ((x == 79 && y == RightP) || (x == 79 && y == RightP + 1) || (x == 79 && y == RightP - 1)) {
                printf("|");
                flag = 1;
            }
