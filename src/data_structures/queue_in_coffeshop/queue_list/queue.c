...
void enqueue(Queue *q, int data)
{
    ...
    {
        q->last_elem->next = new_node; //теперь указатель у бывшего последнего элемента указывает на new_node вместо NULL, то есть он подвинулся
...
int dequeue(Queue *q)
{
    if (q->first_elem == NULL) {printf("Очередь пуста"); return -1;}
